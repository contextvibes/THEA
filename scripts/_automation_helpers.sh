#!/bin/bash
# Filename: scripts/_automation_helpers.sh
#
# -----------------------------------------------------------------------------
# PURPOSE: SINGLE SOURCE OF TRUTH FOR AUTOMATION LOGIC
# -----------------------------------------------------------------------------
# WHAT: This script is a centralized library of shared variables and bash
#       functions. It is not meant to be executed directly.
#
# WHY:  By centralizing reusable code, we achieve several key goals:
#       1.  **Maintainability:** Logic is defined once. Changes are made in
#           one place, reducing errors and effort.
#       2.  **Consistency:** All scripts behave identically for common tasks
#           (e.g., checking for uncommitted changes).
#       3.  **Readability:** Other scripts become shorter and focus on their
#           unique tasks, making them easier to understand.
#
# RULE: If you write a piece of logic that you think could be used in more
#       than one script, it belongs here. This includes constants,
#       validation checks, user prompts, and helper functions.
#
# USAGE: To use these helpers in another script, add the following line near
